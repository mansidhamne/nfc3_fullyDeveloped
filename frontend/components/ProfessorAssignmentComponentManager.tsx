...
const ProfessorAssignmentManager: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const assignmentsData: Assignment[] = [
        { id: 1, title: 'React Basics', subject: 'Web Development', branch: 'Computer Science', status: 'Submitted', studentName: 'John Doe', submissionDate: '2024-08-25',description:'sort' },
        { id: 2, title: 'Data Structures', subject: 'Algorithms', branch: 'Computer Science', status: 'Graded', studentName: 'Jane Smith', submissionDate: '2024-08-24', grade: 'A',description:'sort' },
      ];
      const subjectsData = ['Web Development', 'Algorithms', 'Database Systems'];
      const branchesData = ['Computer Science', 'Information Technology', 'Electronics'];
  ...
  const submitGrade = () => {
    const updatedAssignments = assignments.map((a) =>
      a.id === selectedAssignment?.id ? { ...a, grade, status: 'Graded' } : a
    );
  ...
  const submitNewAssignment = () => {
    ...
    setNewAssignment({ title: '', description: '', subject: '', branch: '', deadline: '' });
  };
  ...
  return (
    <div className="space-y-4 p-4 md:p-8">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={handleNewAssignment} className="bg-blue-500 text-white flex items-center">
        ...
        <TableBody>
          {filteredAssignments.map((assignment) => (
            <TableRow key={assignment.id}>
              ...
              <TableCell>
                <Button onClick={() => handleViewPDF(assignment)} className="mr-2 flex items-center">
                  <Eye className="mr-1" /> View
                </Button>
                <Button onClick={() => handleGrade(assignment)} disabled={assignment.status === 'Graded'} className="flex items-center">
      ...
      <Modal isOpen={isGradeModalOpen} onClose={() => setIsGradeModalOpen(false)}>
        ...
        <Input
          ...
          className="mb-4"
        />
        <Button onClick={submitGrade} className="bg-green-500 text-white">
          Submit Grade
        </Button>
      </Modal>
      ...
      <Modal isOpen={isNewAssignmentModalOpen} onClose={() => setIsNewAssignmentModalOpen(false)}>
        ...
        <Input
          ...
          className="mb-4"
        />
        <Button onClick={submitNewAssignment} className="bg-blue-500 text-white">
          Create Assignment
        </Button>
      </Modal>
    </div>
